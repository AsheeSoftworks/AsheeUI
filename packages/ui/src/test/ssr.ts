/**
 * SSR helper (`TEST-023`, `TEST-034`).
 *
 * Renders a tree to a string so server rendering is exercised without a
 * browser-only code path. Effects do not run during server rendering, so a
 * component that touches browser globals only inside effects is covered by
 * this helper; components that touch browser globals during render fail here.
 *
 * Note: jsdom globals remain installed in this environment, so this helper
 * proves the server render path works rather than simulating a fully
 * browserless process.
 */

import type { ReactElement } from "react";
import { renderToString } from "react-dom/server";

/**
 * Render `element` to an HTML string.
 *
 * @param element - Element to render on the server.
 * @returns The rendered markup.
 */
export function renderToServerString(element: ReactElement): string {
  return renderToString(element);
}
